			...
			<article className="p-6 space-y-4 bg-amber-50 border border-amber-200 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
				...
				>
					<div className="flex flex-col items-center">
						<h3
							className={`${patrick.className} text-2xl mb-2 flex items-center justify-center`}
				...
				<div className="flex flex-wrap gap-2 justify-center">
					{shown.map((tag, i) => (
						<span
							key={`${tag}-${i}`}
							className="text-xs px-2 py-0.5 bg-amber-100 rounded-full text-amber-800 font-medium"
						>
							{tag}
						</span>
					))}
