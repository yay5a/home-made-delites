	...
	if (variant === 'detailed') {
		return (
			<div className='bg-white rounded-md p-4 shadow-sm border border-gray-200'>
				<h3 className='font-medium text-gray-900 mb-1'>Recipe Assistant Status</h3>
				<div className='space-y-2'>
					<div className='flex justify-between text-sm'>
						<span className='text-gray-600'>Remaining Calls:</span>
						<span
							className={`font-medium ${
								quota.remainingCalls < 5 ? 'text-amber-600' : 'text-green-600'
							}`}>
					...
					<div className='flex justify-between text-sm'>
						...
						<span
							className={`font-medium ${
								quota.remainingTokens < 2000 ? 'text-amber-600' : 'text-green-600'
							}`}>
