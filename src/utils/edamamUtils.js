...
// Server-side database utilities removed from client bundle
...
/**
 * Fetch recipes from Edamam API following our schema structure with rate limiting and caching
 * @param {string} query - Search query
 * @param {Object} options - Additional search options
 * @param {boolean} bypassCache - Whether to bypass cache and force a fresh API call
 * @param {boolean} preferDb - Whether to prefer database results over API calls
 * @returns {Promise<Object>} - Validated API response
 */
export async function fetchEdamamRecipes(query, options = {}) {
	try {
		// This function now only fetches from Edamam and does not interact with the DB.
		// Database interactions are handled by API routes.
		...
		// Return the raw Edamam data
		return data;
	} catch (error) {
		console.error('Error fetching from Edamam API:', error);
		// Do not fall back to DB here, let the caller handle errors.
		throw error;
	}
}
